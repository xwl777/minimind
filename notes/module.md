...
4. **model组装以及MiniMindForCausalLM**  
   理论知识没有，主要是一些代码问题  
   * ```   
     ...
     这是用于防御性编程。背景：在 Hugging Face 的新版 transformers 库中，引入了一种新的缓存对象（比如 DynamicCache），这种对象内部会有 layers 这样的属性。MiniMind 是一个简化的实现，它期望 past_key_values 是一个简单的元组列表（List of Tuples），而不期望收到那些复杂的封装对象。这段话的意思是——“如果你传进来的缓存是一个我很陌生的高级对象（带有 layers 属性），我处理不了，为了防止程序崩溃，我干脆把你这个缓存扔掉（置为 None），我就当你是第一次来，重新计算。”  

   * ```  
        aux_loss = sum(block.mlp.aux_loss for block in self.blocks if isinstance(block.mlp, MOEFeedForward))
     ```

     aux_loss是混合专家模型中为了让各个专家都工作而设置的负载均衡损失。暂时还没有实现moe，这段可以直接注释掉令aux_loss=0

   * ```
        self.model.embed_tokens.weight = self.lm_head.weight
     ```

     在 PyTorch 中，所有的 nn.Linear(in, out) 层，其权重 weight 在内存中永远是按照 (out, in) 的形状存储的（这是因为矩阵是行优先存储的，转置能减少运算时的内存开销）。因此，self.model.embed_tokens.weight 和 self.lm_head.weight的形状是一样的。
 ![alt text](linear.png)
