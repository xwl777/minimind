...
9. **lora**
   1.  lora是一种轻量化快速微调大模型的方法，思想是对大模型中的矩阵，不直接微调其参数，而是引入两个低秩矩阵A和B,从而y = xW + xAB，冻结W而训练AB以拟合误差，AB的参数量为r*（dimin + dimout），大大小于W的参数量dimin*dimout（r是很小的秩），从而实现轻量化微调。
   2.  lora的初始化： 一般将A随机初始化而B0初始化。首先A、B不能全为0，否则梯度消失无法训练。同时AB初始应该为0，因为要在原模型基础上微调，AB为0才能保证初始状态下不影响原模型结果，保证稳定性。同时有研究表明，A随机初始化B为0的方案相较于B随机初始化A为0的方案可以应用更大的学习率而不会使模型不稳定![alt text](lora_init.png)
   3.  一些修正。Minimind的实现中，对所以方阵注入了lora，因为这些矩阵往往是GQA中的矩阵，而业界的传统做法是仅微调注意力层而不微调FFN。但是一般而言lora的做法是根据目标模块的名字决定是否注入，同时也提供了微调FFN层中的矩阵的可能，对此进行了一些修改。同时minimind中```lora(weight.shape[0],weight.shape[1])```的做法在矩阵不是方阵是会引入严重的维度错误（weight是转置存储的），已修正。
